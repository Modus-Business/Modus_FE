...
export async function createNotice(
  ...
  try {
    const response = await teacherNoticesClient.post<BackendEnvelope<NoticeItemResponseData>>(
      "/notices",
      body,
      {
        headers: {
    ...
    if (response.status < 200 || response.status >= 300 || !payload?.data) {
      return {
        ...
        message: getErrorMessage(payload, "공지 작성에 실패했습니다."),
      };
    }
...
export async function getNoticesByClass(
  ...
  try {
    const response = await teacherNoticesClient.get<BackendEnvelope<NoticeListResponseData>>(
