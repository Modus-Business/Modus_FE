...
export function ClassCard({ href, name, code, schedule, description, metaLabel, metaValue, footerLabel, footerValue }: ClassCardProps) {
  return (
    <Card className="relative flex h-full flex-col border-border/80 bg-white/95">
      <Button asChild size="icon" variant="ghost" className="absolute top-4 right-4 z-10 size-9 sm:top-5 sm:right-5">
        <Link href={href} aria-label={`${name} 열기`}>
          <ArrowUpRight className="size-4" />
        </Link>
      </Button>
      <CardHeader className="space-y-4 pr-12 sm:pr-16">
        {code ? <Badge variant="secondary">{code}</Badge> : null}
        <div className="min-w-0">
          <CardTitle className="break-keep text-lg leading-snug sm:text-xl">{name}</CardTitle>
      ...
      <CardFooter className="mt-auto pr-4 sm:pr-16">
